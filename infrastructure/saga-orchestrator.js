...
class CommercialSagaOrchestrator {
  constructor(options = {}) {
    ...
    // Fusion 集成：动态加载 Fusion Skill 作为额外 Stage
    this.fusionOrchestrator = options.fusionOrchestrator || null;
    this.fusionSkillIds = options.fusionSkillIds || null; // 指定加载哪些 Skill，null=全部
  }
   ...
   * 将卓越/暴风战斧 Skill 包装为原生 Saga Stage
   */
  async registerFusionSkills(fusionOptions = {}) {
    if (!this.fusionOrchestrator) {
      console.log('[CommercialSaga] ℹ️ 未配置 Fusion Orchestrator，跳过 Skill 注入');
      return this;
    }

    // 确保 Fusion Orchestrator 已初始化
    if (!this.fusionOrchestrator.initialized) {
      await this.fusionOrchestrator.initialize();
    }

    const skillInstances = this.fusionOrchestrator.skillInstances;
    const dynamicStages = this.fusionOrchestrator.dynamicStageDefinitions;
    
    // 过滤 Skill：如果指定了 fusionSkillIds，只注册这些
    const skillIdsToRegister = this.fusionSkillIds 
      ? Array.from(skillInstances.keys()).filter(id => this.fusionSkillIds.includes(id))
      : Array.from(skillInstances.keys());

    console.log(`[CommercialSaga] 🔧 注册 Fusion Skill: ${skillIdsToRegister.length} 个`);

    for (const skillId of skillIdsToRegister) {
      const instance = skillInstances.get(skillId);
      const stageDef = dynamicStages[Object.keys(dynamicStages).find(k => dynamicStages[k]._skillId === skillId)];
      
      if (!instance || !stageDef) {
        console.warn(`[CommercialSaga] ⚠️ Skill ${skillId} 未找到，跳过`);
        continue;
      }

      // 将 Fusion Stage 映射为原生 Saga Stage
      const sagaStageConfig = {
        id: stageDef.id,
        name: stageDef.name || skillId,
        phase: stageDef.phase || 'production',
        blocking: stageDef.blocking !== false,
        required: stageDef.required !== false,
        timeoutMs: stageDef.timeoutMs || 30000,
        retryPolicy: stageDef.retryPolicy || { maxAttempts: 1, backoffMs: 1000 },
        fallback: stageDef.fallback || { strategy: 'skip' },
        compensate: stageDef.compensate || null,
        _skillId: skillId,
        _isFusion: true  // 标记为 Fusion Stage
      };

      // 包装 Skill.execute 为 Saga Stage 执行函数
      const executeFn = async (input, context) => {
        const traceId = context.traceId || this.traceId || `fusion_${Date.now()}`;
        
        // 调用 Fusion Skill 的 execute 方法
        const result = await instance.execute(input, { traceId, ...context });
      ...
      this.registerStage(sagaStageConfig, executeFn);
      console.log(`[CommercialSaga] ✅ Fusion Skill 注册: ${skillId} -> ${stageDef.id}`);
    }
  ...
  async execute(input, options = {}) {
    ...
    // 构建 Stage 执行序列：原生 Stage + Fusion Stage
    let stageOrder = options.stageOrder || Array.from(this.stages.keys());
    
    // 如果配置了 Fusion，自动将 Fusion Stage 追加到序列末尾
    if (this.fusionOrchestrator && this.fusionOrchestrator.initialized && !options.stageOrder) {
      const fusionStageIds = Object.keys(this.fusionOrchestrator.dynamicStageDefinitions);
      // 只追加尚未在 stageOrder 中的 Fusion Stage
      for (const fusionStageId of fusionStageIds) {
        if (!stageOrder.includes(fusionStageId)) {
          stageOrder.push(fusionStageId);
        }
      }
    }
    ...
    if (this.fusionOrchestrator) {
      console.log(`[CommercialSaga] 🔧 Fusion Stage: ${stageOrder.filter(id => id.startsWith('STAGE-FUSION')).length} 个`);
    }
