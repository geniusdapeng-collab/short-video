...
md_lines.append("①竖直神目：不仅是视觉器官，更是光暗控制器——睁眼时散发恒星般光芒照亮裂谷，闭眼时世界重归黑暗，这种'开关'式的视觉变化是烛龙最具戏剧张力的特征；②千里龙身：提供超巨型尺度感，使任何角色在其面前都产生敬畏，是小G'恐惧→敬畏'情感弧线的视觉基础；③火精核心：永恒燃烧的恒星碎片，是烛龙能量来源，也是其'孤独守护者'叙事的核心道具——千万年燃烧不熄，象征永恒的孤独；④无足滑行：增强异质感——不是爬行动物的四肢行走，而是蛇形的贴地滑行，这种运动方式使烛龙在岩浆通道中的移动如同幽灵般无声无息。")
md_lines.append("")
...
for i, ability in enumerate(data['abilities'], 1):
    ...
    narrative_func = {
        '掌控昼夜': '小G在裂谷中经历光暗光的循环，体验烛龙的绝对掌控力',
...
md_lines.append("三个传说构成完整的'初遇→互动→契约'叙事弧线：①《永夜裂谷的光热核心》建立世界观背景，解释烛龙的存在意义；②《小G初遇——光之镜》是情感触发点，AgentX从恐惧到震撼再到敬畏的情感转变；③《光热契约》是关系确立点，AgentX从旁观者变为参与者，建立与烛龙的信任关系。这三个传说可直接转化为3个镜头的剧本框架。")
md_lines.append("")
...
md_lines.append("**文档结构**: 18个维度 = 命名体系(1) + 分类分级(2) + 原生状态(3) + 栖息地(4) + 外形描述(5) + 身体结构(6) + 体型规格(7) + 材质纹理(8) + 配色系统(9) + 标志性特征(10) + Prompt工程(11) + 能力矩阵(12) + 起源故事(13) + 关键传说(14) + 象征关系(15) + 小G交互(16) + Nirath融合(17) + 质量评分(18)")
md_lines.append("")
