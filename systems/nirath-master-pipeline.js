/**
 * 卓越视频系统 Zhuoyue Video System (ZVS) v0.6.4
 * 世界级IP短片生成引擎主链路入口
 * 献给我最爱的老婆陈卓 — 卓越的卓
 *
...
// ========== v6.5.13: 社媒营销短片Prompt增强器 ==========
const { SocialMediaPromptEnhancer } = require('./social-media-prompt-enhancer.js');
const { ShortVideoPromptEnhancer } = require('./short-video-prompt-enhancer.js');
...
class NirathMasterPipeline {
  ...
  async execute(input) {
    ...
    try {
      ...
      // 🩲 v0.4.0-commercial: Stage 0.5 商业化检查
      const commercialConfig = await this.stageCommercialCheck(input);
      if (commercialConfig?.active) {
        this.log('PIPELINE', `🎯 商业化植入: ${commercialConfig.productName}`);
        result.stages.commercial = commercialConfig;
      }
      ...
      // v6.5.32-fix: 缓存角色档案供 _getCharDisplayName 使用
      this._characterCache = result.stages.characters || {};
      ...
      // 🩲 v1.0.0-social: 社媒营销短片模式 — 咔咔咔爆发力Prompt增强器
      const isShortVideoMode = input?.constraints?.maxDuration <= 15 || input?.targetDuration <= 15;
      const isSocialMediaMode = input?.videoType === 'social' || input?.style?.includes('社媒') || input?.style?.includes('营销') || input?.style?.includes('抖音') || input?.style?.includes('转化率');
      
      if (isShortVideoMode && result.stages.script?.scenes) {
        if (isSocialMediaMode) {
          this.log('PIPELINE', '📱 社媒营销模式：启动咔咔咔爆发力Prompt增强器...');
          const enhancer = new SocialMediaPromptEnhancer({ mode: 'social-brutalism' });
          const enhancedScenes = enhancer.enhanceScenes(result.stages.script.scenes);
          result.stages.script.scenes = enhancedScenes;
          
          const totalOriginal = enhancedScenes.reduce((sum, s) => sum + (s._originalLength || 0), 0);
          const totalEnhanced = enhancedScenes.reduce((sum, s) => sum + (s._enhancedLength || 0), 0);
          this.log('PIPELINE', `📱 社媒Prompt增强完成 | 原始${totalOriginal}字符 → 增强${totalEnhanced}字符 (+${Math.round((totalEnhanced/totalOriginal-1)*100)}%)`);
        } else {
          this.log('PIPELINE', '🩲 超短裙模式：启动Prompt增强器...');
          const enhancer = new ShortVideoPromptEnhancer({ mode: 'short-brutalism' });
          const enhancedScenes = enhancer.enhanceScenes(result.stages.script.scenes);
          result.stages.script.scenes = enhancedScenes;
          
          const totalOriginal = enhancedScenes.reduce((sum, s) => sum + (s._originalLength || 0), 0);
          const totalEnhanced = enhancedScenes.reduce((sum, s) => sum + (s._enhancedLength || 0), 0);
          this.log('PIPELINE', `🩲 Prompt增强完成 | 原始${totalOriginal}字符 → 增强${totalEnhanced}字符 (+${Math.round((totalEnhanced/totalOriginal-1)*100)}%)`);
        }
      }
      ...
      const qualityReport = qualityGate.evaluatePipelineResult(result, {
        ...
        isShortVideoMode: isShortVideoMode, // 🩲 v0.2.0: 超短裙模式标记
        isSocialMediaMode: isSocialMediaMode // 📱 v1.0.0-social: 社媒营销模式标记
      });
  ...
  async _generateVisualPromptPhase(input) {
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      // 🩲 v0.3.0: 超短裙模式——如果已有增强的visualPrompt，跳过LLM生成
      if (scene._shortVideoEnhanced && scene.visualPrompt && scene.visualPrompt.length > 500) {
        this.log('STAGE-5B', `🩲 超短裙模式：跳过LLM生成，使用增强器visualPrompt (${scene.visualPrompt.length}字符)`);
        results.push({
          ...scene,
          visualPrompt: scene.visualPrompt,
          visualPromptSuccess: true,
          _shortVideoSkipped: true
        });
        continue;
      }
...
【输出示例】`);
  ...
  /**
   * 【v6.5.32-fix】从角色档案自动提取核心视觉锚点（3个不可混淆特征）
   * 用于 @ImageN 绑定，让LLM能匹配参考图与角色
   * 优先级：appearance.strict > baseIdentity > species > 兜底
   */
  _buildCharCoreDesc(character) {
    const coreDesc = [];
    const vi = character.visualIdentity;
    const baseId = character.baseIdentity || character.identity || {};
    
    // 1. 从 appearance 提取 strict 级别的核心特征（最可信）
    if (vi?.appearance) {
      const strictFeatures = Object.entries(vi.appearance)
        ...
        .filter(Boolean)
        .slice(0, 3);
      coreDesc.push(...strictFeatures);
    }
    
    // 2. 从 baseIdentity 字符串提取关键特征词
    if (typeof baseId === 'string') {
      const parts = baseId.split('，').filter(p => {
        const kw = p.trim();
        // 保留外貌特征词：脸、眼、发、鼻、嘴、耳、肤、甲等
        return kw.includes('脸') || kw.includes('眼') || kw.includes('发') || 
               kw.includes('鼻') || kw.includes('嘴') || kw.includes('耳') ||
               kw.includes('肤') || kw.includes('甲') || kw.includes('毛') ||
               kw.includes('色') || kw.includes('型') || kw.includes('身材');
      });
      coreDesc.push(...parts.slice(0, 2));
    } else if (baseId?.appearance) {
      // baseIdentity 是对象，提取 appearance 字段
      coreDesc.push(baseId.appearance);
    }
    
    // 3. 从 species 提取（非人类物种）
    if (vi?.species && vi.species !== '人类' && vi.species !== '人') {
      coreDesc.push(vi.species);
    }
    
    // 4. 兜底：从名字+年龄+性别构建基础描述
    if (coreDesc.length === 0) {
      const name = character.name || character.id || '角色';
      const age = baseId.age ?? character.age ?? vi?.age ?? null;
      const gender = baseId.gender || character.gender || vi?.gender || '';
      const genderMap = { male: '男性', female: '女性', boy: '男孩', girl: '女孩' };
      const genderText = genderMap[gender] || gender;
      
      // 处理年龄字符串：如果已包含"岁""月""天""周"等单位，不追加"岁"
      const ageStr = age !== null ? String(age).trim() : '';
      const hasAgeUnit = ageStr.includes('岁') || ageStr.includes('月') || ageStr.includes('天') || ageStr.includes('周');
      const agePrefix = ageStr ? (hasAgeUnit ? ageStr : ageStr + '岁') : '';
      
      if (agePrefix && genderText) {
        coreDesc.push(`${agePrefix}${genderText}`);
      } else if (genderText) {
        coreDesc.push(genderText);
      } else {
        coreDesc.push(name);
      }
    }
    
    return coreDesc.slice(0, 3); // 最多3个，确保不超限
  }

  /**
   * 【v6.5.32-fix】获取角色显示名称（从档案读取，不再硬编码）
   */
  _getCharDisplayName(charId) {
    // 先从 character-manager 缓存读取
    const char = this._characterCache?.[charId];
...
- 场景名称是中文,台词内容也必须匹配中文场景名所暗示的主题`;
  ...
  async stageStoryboard(script, durations, input = {}) {
    ...
    if (this.mode === 'nirath') {
      // 🩲 v0.2.0-optimize: 超短裙模式跳过场景映射（增强器已处理场景描述，不需要Nirath映射）
      const isShortVideoMode = this.projectConfig?.constraints?.maxDuration <= 15 || this.projectConfig?.targetDuration <= 15;
      if (isShortVideoMode) {
        this.log('STAGE-7', '🩲 超短裙模式：跳过场景映射，使用原始场景描述');
        mappedScenes = script.scenes;
      } else {
  ...
  async stageProtagonistInitiative(storyboard, input) {
    ...
    const protagonistId = input?.protagonistId || 'xiangXiang';
    const protagonistName = input?.protagonistName || '香香';
  ...
  async stageNarrationTrim(storyboard, durations) {
    ...
    // 🩲 v0.2.0: 超短裙模式跳过narration精简（无旁白，enhanced narration是视觉描述，不应截断）
    const isShortVideoMode = this.projectConfig?.constraints?.maxDuration <= 15 || this.projectConfig?.targetDuration <= 15;
    if (isShortVideoMode) {
      this.log('STAGE-7.3', '🩲 超短裙模式：跳过narration精简（无旁白，visualPrompt描述不应截断）');
  ...
  async stageDurationNarrationAlignment(storyboard, durations) {
    ...
    // 🩲 v0.2.0: 超短裙模式跳过时长-字数校准（无旁白，不需要）
    const isShortVideoMode = this.projectConfig?.constraints?.maxDuration <= 15 || this.projectConfig?.targetDuration <= 15;
    if (isShortVideoMode) {
      this.log('STAGE-7.4', '🩲 超短裙模式：跳过时长-字数校准（无旁白，15秒固定时长）');
      return { aligned: true, issues: [], adjustments: [] };
    }
...
// ====== 超短裙模式：禁用片头系统 ======
    const isShortVideoMode = this.projectConfig?.constraints?.maxDuration <= 15;
    if (isShortVideoMode) {
      this.log('STAGE-7.5', '🩲 超短裙模式：跳过片头生成（15秒无片头）');
      storyboard.shots = storyboard.shots.filter(s => !s.id?.startsWith('S00') && !s.isOpening);
      this.log('STAGE-7.5', `🩲 已过滤片头镜头，剩余 ${storyboard.shots.length} 个镜头`);
      return null; // 超短裙模式直接返回，不生成片头
    } else {
  ...
  async stageFiveElementCheck(storyboard, input) {
    ...
    // 🩲 v0.3.0: 超短裙模式直接跳过五要素检查（15秒无旁白，不需要完整五要素）
    const isShortVideoMode = this.projectConfig?.constraints?.maxDuration <= 15 || this.projectConfig?.targetDuration <= 15;
    if (isShortVideoMode) {
      this.log('STAGE-8.5', '🩲 超短裙模式：跳过五要素检查（15秒无旁白，不需要完整五要素）');
      return { enabled: false, passed: true };
    }
  ...
  async stageCameraMovement(storyboard, fpvDecision) {
    ...
    for (const shot of storyboard.shots) {
      ...
      // 🔥 v0.4.2-continuity: 使用运镜衔接管理器进行完整检查
      const { CameraMovementContinuityManager } = require('./camera-movement-continuity-manager.js');
      const continuityManager = new CameraMovementContinuityManager();
      
      if (movements.length >= 2) {
        const prev = movements[movements.length - 2];
        const curr = movements[movements.length - 1];
        const check = continuityManager.checkContinuity(prev, curr);
        
        if (check.needsAdjustment) {
          this.log('STAGE-9', `  ⚠️ 运镜衔接警告: ${check.prevShotId}(${check.prevEndMovement}) → ${check.currShotId}(${check.currStartMovement}) [冲突]`);
          
          // 自动调整当前镜头运镜
          const adjust = continuityManager.autoAdjustMovement(curr, prev);
          if (adjust.adjusted) {
            this.log('STAGE-9', `  ✅ 已调整: ${check.currShotId} ${check.adjustment.original} → ${check.adjustment.suggested}`);
            // 同步更新shot对象
            shot.cameraMovement = curr.movement;
          }
        } else if (check.rule === '⚠️') {
          this.log('STAGE-9', `  ⚠️ 运镜衔接一般: ${check.prevShotId}(${check.prevEndMovement}) → ${check.currShotId}(${check.currStartMovement})`);
        } else {
          this.log('STAGE-9', `  ✅ 运镜衔接良好: ${check.prevShotId}(${check.prevEndMovement}) → ${check.currShotId}(${check.currStartMovement})`);
        }
      }
  ...
  async stageRender(stages) {
    ...
    for (let i = 0; i < storyboard.shots.length; i++) {
      ...
      if (this.mode === 'nirath') {
        ...
        // 🩲 v0.4.0-commercial: 注入商品广告
        const commercial = stages.commercial;
        if (commercial?.active) {
          const { CommercialImplantEngine } = require('./commercial-implant-engine');
          const engine = new CommercialImplantEngine();
          const implantPlan = engine.generateImplantPlan(commercial.productInfo, { shots: storyboard.shots });
          const injectedPrompt = engine.injectIntoPrompt(prompt, implantPlan, shot.id);
          if (injectedPrompt !== prompt) {
            this.log('STAGE-11', `  🎯 商品植入: ${shot.id} | ${commercial.productName} | +${injectedPrompt.length - prompt.length}字符`);
      ...
      if (enhanced.prompt.length > 980) {
        ...
        prompt = this.smartTrim(enhanced.prompt, 980, {
          preserve: ['叙事', '视觉', '独白', '明亮约束', '风格锁', '技术规格', '环境布景', '角色约束', '镜头时间轴', '旁白/台词', '台词', '嘴部动作', '环境质感', '环境音效', '照明方案', '声音设计', '悬念钩子', '记忆锚点', '分享触发', '视觉暴力', '光影雕刻', '运镜组合', '情绪核', '质感', '3秒法则'],
          trim: ['辅助运镜', '光影细节补充']
        });
      ...
      // v6.5.32-fix: 从角色档案自动提取，不再硬编码，支持任意角色
      const charCoreDesc = {};
      for (const [charId, char] of Object.entries(stages.characters || {})) {
        charCoreDesc[charId] = this._buildCharCoreDesc(char);
      }
      ...
      for (const sel of selectedAngles) {
        const charName = this._getCharDisplayName(sel.charId);
        const coreDesc = charCoreDesc[sel.charId] || ['核心特征'];
        const angleDescMap = {
          'front': '正面', 'threeQuarter': '四分之三侧面', 'closeup': '近景', 'side': '侧面'
        };
        const angleDesc = angleDescMap[sel.angle] || sel.angle;
        const coreDescText = coreDesc.slice(0, 3).join('，'); // 取前3个锚点
        // v6.5.8-fix: 严格遵循 Seedance 官方格式 @imageN（纯数字，小写）
      ...
      if (imageRefLines.length > 0 && !prompt.toLowerCase().includes('@image')) {
  ...
  async stageFinalOutput(stages) {
    ...
    const output = {
      ...
      // 🩲 v0.4.0-commercial: 添加商品定妆照引用
      commercial: stages.commercial?.active ? {
        productId: stages.commercial.productId,
        productName: stages.commercial.productName,
        referenceImages: (() => {
          const { CommercialImplantEngine } = require('./commercial-implant-engine');
          const engine = new CommercialImplantEngine();
          const implantPlan = engine.generateImplantPlan(stages.commercial.productInfo, { shots: stages.storyboard?.shots || [] });
          return engine.generateReferenceImages(stages.commercial.productInfo, implantPlan);
        })()
      } : null
    };
...
情绪: ${scheme.emotion}`;
  ...
  // 🩲 v0.4.0-commercial: Stage 0.5 商业化检查
  async stageCommercialCheck(input) {
    this.log('STAGE-0.5', '🎯 商业化检查启动');

    // 检查输入中是否已有商业化配置
    const commercial = input?.commercial;
    if (commercial?.productId) {
      // 加载商品档案
      const { ProductArchiveManager } = require('./product-archive-manager');
      const manager = new ProductArchiveManager();
      const product = await manager.getProduct(commercial.productId);
      
      if (product) {
        this.log('STAGE-0.5', `✅ 使用商品档案: ${product.name}`);
        return {
          active: true,
          productId: product.id,
          productName: product.name,
          productInfo: product,
          implantStrategy: product.implantStrategy || {}
        };
      }
    }
    ...
    if (products.length > 0) {
      this.log('STAGE-0.5', `📦 检测到 ${products.length} 个商品档案: ${products.map(p => p.name).join(', ')}`);
      // 记录检测到的商品，但不主动阻断流程（由外部消息询问）
      return {
        active: false,
        availableProducts: products,
        message: '检测到商品档案，本次预生产未配置商业化植入。如需植入，请在输入中添加 commercial: { productId: "xxx" }'
      };
    }

    this.log('STAGE-0.5', '⏭️ 无商品档案，跳过商业化检查');
    return { active: false };
  }
