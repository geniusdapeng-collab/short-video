...
function generateAct2_Development({ startTime, duration, protagonist, beast, episodeSummary, mood }) {
  ...
  const protagonistName = protagonist?.name || '香香';
...
function combineActs(act1, act2, act3, config) {
  ...
  const protagonistId = config.protagonistId || 'xiangXiang';
  ...
  const imageRefLines = [];
  let imageIdx = 1;
  // v6.5.32-fix: 扩展核心特征映射，支持所有角色（不再硬编码仅限小G/饕餮）
  const charCoreDesc = {
    'xiaoG': ['银灰装甲', '东亚面孔短发', '年轻男性'],
    'tao-tie': ['碳化硅质甲壳', '腋下双眼', '巨口能量涡流'],
    'taotie': ['碳化硅质甲壳', '腋下双眼', '巨口能量涡流'],
    'xiangXiang': ['7个月女孩', '圆脸大眼', '可爱笑容'],
    'xiaoZhuo': ['35岁女性', '马尾休闲装', '温柔母亲']
  };
  ...
  for (const charId of charIds) {
    ...
    if (portraitPath) {
      ...
      const charName = charId === 'xiaoG' ? '小G' : (charId === 'tao-tie' || charId === 'taotie' ? '饕餮' : (charId === 'xiangXiang' ? '香香' : (charId === 'xiaoZhuo' ? '小卓' : charId)));
      const coreDesc = charCoreDesc[charId] || ['核心特征'];
      const coreDescText = coreDesc.slice(0, 3).join('，');
      // v6.5.10-fix: 严格遵循 Seedance 官方格式 @imageN（纯数字，小写）
...
if (require.main === module) {
  ...
  console.log('\n✅ 通用片头系统 v3.0 测试完成');
  console.log('\n⚠️ 注意:如果Prompt长度>980,系统会自动渐进裁剪(技术规格→运镜→风格锁),但叙事和角色内容永远优先保留!');
  console.log('   裁剪顺序:1)技术规格精简 2)运镜精简 3)风格锁精简(叙事和角色永不裁剪)');
  console.log('   如果裁剪后仍>980,说明输入剧情太长,需要精简episodeSummary!');
}
