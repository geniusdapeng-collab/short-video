...
class OrientPrimordialCoreV24 {
  ...
  buildPromptV3(params = {}) {
    ...
    // 🩲 v0.3.0: 超短裙模式——如果script包含增强器标记，直接使用，跳过模板生成
    const isShortVideoPrompt = processedScript.includes('【超短裙');
    if (isShortVideoPrompt) {
      console.log('[Core-v24.3] 🩲 超短裙模式：检测到增强器模板，直接使用原始内容');
      // 不截断，使用完整内容
      return {
        prompt: `【视觉】${processedScript}`,
        styleCalibrated: false,
        _shortVideoMode: true
      };
    }
