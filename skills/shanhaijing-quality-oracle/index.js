...
/**
 * 质量神谕 — Stormaxe Skill Wrapper
 * 原始模块: quality-oracle.js
 */

const path = require('path');
const { SkillBase } = require('../skill-base');

const STORM_ROOT = path.join(__dirname, '..', '..', '..', 'StormaxeAIVideoSystem', 'skills', 'shanhaijing-quality-oracle');

class QualityOracleSkill extends SkillBase {
  constructor(options = {}) {
    super({
      name: 'shanhaijing-quality-oracle',
      version: '1.0.0',
      description: '五维十八指标评测（S+~C 六级评分）',
      category: 'stormaxe',
      ...options
    });
  ...
  async onInitialize(context) {
    try {
      const StormModule = require(path.join(STORM_ROOT, 'quality-oracle.js'));
      
      // 处理不同的导出格式
      let StormClass = StormModule;
      if (StormModule.QualityOracle) {
        StormClass = StormModule.QualityOracle;
      ...
      console.log(`[shanhaijing-quality-oracle] ✅ 原始模块加载成功`);
      return true;
    } catch (e) {
      console.warn(`[shanhaijing-quality-oracle] ⚠️ 原始模块加载失败: ${e.message}`);
      // 降级：创建 mock 实例
  ...
  async onExecute(input, context) {
    const traceId = (context && context.traceId) || `oracle_${Date.now()}`;
    
    if (!this.stormInstance) {
      throw new Error('shanhaijing-quality-oracle: 原始模块未初始化');
    }

    // 透传输入，让原始模块自行处理
    // 暴风战斧 Skill 的接口各不相同，这里采用通用透传策略
    const result = { _skillId: 'shanhaijing-quality-oracle', _timestamp: Date.now() };
...
module.exports = { QualityOracleSkill };
