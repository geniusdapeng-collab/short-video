...
const STORM_ROOT = path.join(__dirname, '..', '..', '..', 'StormaxeAIVideoSystem', 'skills', 'shanhaijing-director');

class ShanhaiDirectorSkill extends SkillBase {
  constructor(options = {}) {
    super({
      name: 'shanhaijing-director',
      version: '1.0.0',
      description: '多 Agent 协同导演（构图/光影/角色）',
      category: 'stormaxe',
      ...options
    });
  ...
  async onInitialize(context) {
    try {
      const StormModule = require(path.join(STORM_ROOT, 'director.js'));
      
      // 处理不同的导出格式
      let StormClass = StormModule;
      if (StormModule.ShanhaiDirector) {
        StormClass = StormModule.ShanhaiDirector;
      ...
      console.log(`[shanhaijing-director] ✅ 原始模块加载成功`);
      return true;
    } catch (e) {
      console.warn(`[shanhaijing-director] ⚠️ 原始模块加载失败: ${e.message}`);
      // 降级：创建 mock 实例
  ...
  async onExecute(input, context) {
    const traceId = (context && context.traceId) || `director_${Date.now()}`;
    
    if (!this.stormInstance) {
      throw new Error('shanhaijing-director: 原始模块未初始化');
    }

    // 透传输入，让原始模块自行处理
    // 暴风战斧 Skill 的接口各不相同，这里采用通用透传策略
    const result = { _skillId: 'shanhaijing-director', _timestamp: Date.now() };
...
module.exports = { ShanhaiDirectorSkill };
