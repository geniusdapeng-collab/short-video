...
const STORM_ROOT = path.join(__dirname, '..', '..', '..', 'StormaxeAIVideoSystem', 'skills', 'shanhaijing-soul-forge');

class SoulForgeSkill extends SkillBase {
  constructor(options = {}) {
    super({
      name: 'shanhaijing-soul-forge',
      version: '1.0.0',
      description: '三层灵魂模型（本能/人性/灵性）',
      category: 'stormaxe',
      ...options
    });
  ...
  async onInitialize(context) {
    try {
      const StormModule = require(path.join(STORM_ROOT, 'soul-forge.js'));
      
      // 处理不同的导出格式
      let StormClass = StormModule;
      if (StormModule.SoulForge) {
        StormClass = StormModule.SoulForge;
      ...
      console.log(`[shanhaijing-soul-forge] ✅ 原始模块加载成功`);
      return true;
    } catch (e) {
      console.warn(`[shanhaijing-soul-forge] ⚠️ 原始模块加载失败: ${e.message}`);
      // 降级：创建 mock 实例
  ...
  async onExecute(input, context) {
    const traceId = (context && context.traceId) || `forge_${Date.now()}`;
    
    if (!this.stormInstance) {
      throw new Error('shanhaijing-soul-forge: 原始模块未初始化');
    }

    // 透传输入，让原始模块自行处理
    // 暴风战斧 Skill 的接口各不相同，这里采用通用透传策略
    const result = { _skillId: 'shanhaijing-soul-forge', _timestamp: Date.now() };
