...
/**
 * 角色灵魂铸造器 — Stormaxe Skill Wrapper
 * 原始模块: persona-vault.js
 */

const path = require('path');
const { SkillBase } = require('../skill-base');

const STORM_ROOT = path.join(__dirname, '..', '..', '..', 'StormaxeAIVideoSystem', 'skills', 'shanhaijing-persona-vault');

class PersonaVaultSkill extends SkillBase {
  constructor(options = {}) {
    super({
      name: 'shanhaijing-persona-vault',
      version: '1.0.0',
      description: '角色灵魂铸造（Want/Need/Lie/Wound 四维度）',
      category: 'stormaxe',
      ...options
    });
  ...
  async onInitialize(context) {
    try {
      const StormModule = require(path.join(STORM_ROOT, 'persona-vault.js'));
      
      // 处理不同的导出格式
      let StormClass = StormModule;
      if (StormModule.PersonaVault) {
        StormClass = StormModule.PersonaVault;
      ...
      console.log(`[shanhaijing-persona-vault] ✅ 原始模块加载成功`);
      return true;
    } catch (e) {
      console.warn(`[shanhaijing-persona-vault] ⚠️ 原始模块加载失败: ${e.message}`);
      // 降级：创建 mock 实例
  ...
  async onExecute(input, context) {
    const traceId = (context && context.traceId) || `vault_${Date.now()}`;
    
    if (!this.stormInstance) {
      throw new Error('shanhaijing-persona-vault: 原始模块未初始化');
    }

    // 透传输入，让原始模块自行处理
    // 暴风战斧 Skill 的接口各不相同，这里采用通用透传策略
    const result = { _skillId: 'shanhaijing-persona-vault', _timestamp: Date.now() };
...
module.exports = { PersonaVaultSkill };
