...
const STORM_ROOT = path.join(__dirname, '..', '..', '..', 'StormaxeAIVideoSystem', 'skills', 'shanhaijing-voice-craft');

class VoiceCraftSkill extends SkillBase {
  constructor(options = {}) {
    super({
      name: 'shanhaijing-voice-craft',
      version: '1.0.0',
      description: '声纹提取与注入（潜台词/失控/沉默）',
      category: 'stormaxe',
      ...options
    });
  ...
  async onInitialize(context) {
    try {
      const StormModule = require(path.join(STORM_ROOT, 'voice-craft.js'));
      
      // 处理不同的导出格式
      let StormClass = StormModule;
      if (StormModule.VoiceCraft) {
        StormClass = StormModule.VoiceCraft;
      ...
      console.log(`[shanhaijing-voice-craft] ✅ 原始模块加载成功`);
      return true;
    } catch (e) {
      console.warn(`[shanhaijing-voice-craft] ⚠️ 原始模块加载失败: ${e.message}`);
      // 降级：创建 mock 实例
  ...
  async onExecute(input, context) {
    const traceId = (context && context.traceId) || `craft_${Date.now()}`;
    
    if (!this.stormInstance) {
      throw new Error('shanhaijing-voice-craft: 原始模块未初始化');
    }

    // 透传输入，让原始模块自行处理
    // 暴风战斧 Skill 的接口各不相同，这里采用通用透传策略
    const result = { _skillId: 'shanhaijing-voice-craft', _timestamp: Date.now() };
...
module.exports = { VoiceCraftSkill };
